    ...
    if(isset($_POST["btn_submit_register"]) && !empty($_POST["btn_submit_register"])){
        ...
        if(isset($_POST["captcha"]) && !empty($captcha)){
            ...
            if(isset($_POST["first_name"])){
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error'>Missing box with first name</p>";
            ...
            if(isset($_POST["last_name"])){
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error'>Missing box with last name</p>";
            ...
            if(isset($_POST["email"])){
                ...
                if(!empty($email)){
                    ...
                    if( !preg_match($reg_email, $email)){
                        // Сохраняем в сессию сообщение об ошибке. 
                        $_SESSION["error_messages"] .= "<p class='mesage_error' >You entered a wrong email</p>";
                    ...
                    $result_query = $mysqli->query("SELECT `email` FROM `users_list` WHERE `email`='".$email."'");
                    ...
                    if($result_query->num_rows == 1){
                        ...
                        if(($row = $result_query->fetch_assoc()) != false){
                                ...
                                // Сохраняем в сессию сообщение об ошибке. 
                                $_SESSION["error_messages"] .= "<p class='mesage_error' > A user with such a mailing address is already registered </p>";
                            ...
                            // Сохраняем в сессию сообщение об ошибке. 
                            $_SESSION["error_messages"] .= "<p class='mesage_error' > Error in the query to the database </p>";
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error'>Missing box for input email</p>";
            ...
            if(isset($_POST["password"])){
                ...
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error'>Missing box for input password</p>";
            ...
            $result_query_insert = $mysqli->query("INSERT INTO `users_list` (first_name, last_name, email, password) VALUES ('".$first_name."', '".$last_name."', '".$email."', '".$password."')");

            if(!$result_query_insert){
                // Сохраняем в сессию сообщение об ошибке. 
                $_SESSION["error_messages"] .= "<p class='mesage_error' >Error requesting user to add to database </p>";
            ...
            $mysqli->close();         
                ...
                exit("<p><strong>Ошибка!</strong> There is no verification code, that is, a CAPTCHA code. You can go to <a href=".$address_site."> main page </a>.</p>");
                    ...
                    exit("<p><strong>Error!</strong> You have come directly to this page, so there is no data to process. You can go to <a href=".$address_site."> main page </a>.</p>");
                }
?>